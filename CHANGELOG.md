...
- **Improved `discover --auto-import` output**:
  - Already imported files now show as "already imported" instead of errors
  - Added clean progress display with inline status for each file
  - Summary shows counts for new, already imported, and failed files
  - Reduced verbose output during bulk imports

### Fixed

- **Transaction warning**: Fixed spurious "failed to rollback transaction" warning
